...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
/* Application flash region to be checked */
#define APP_START_ADDR   0x08004000UL
#define APP_END_ADDR     0x0800F7FBUL   /* last byte included in CRC */
#define APP_CRC_ADDR     0x0800F7FCUL   /* 4-byte stored CRC */
/* STM32F103C8T6 SRAM: 20 KB at 0x2000_0000 – 0x2000_4FFF */
#define SRAM_START_ADDR        0x20000000UL
#define SRAM_SIZE_BYTES        (20U * 1024U)
#define SRAM_END_ADDR          (SRAM_START_ADDR + SRAM_SIZE_BYTES)  /* 0x20005000 */
#define SRAM_STACK_RESERVE     1024U   /* keep top 1 KB safe for stack, extra margin */
/* USER CODE END PD */
...
__attribute__((noinline, optimize("O0"))) static uint8_t RAM_SimpleTest(void)
{
	...
	volatile uint32_t sp_now = __get_MSP();
	volatile uint32_t test_end_addr = SRAM_END_ADDR;
	...
	p   = (uint32_t *)SRAM_START_ADDR;
	end = (uint32_t *)test_end_addr;
...
static uint32_t APP_CalcCrc(void)
{
	uint32_t length_bytes = (APP_END_ADDR - APP_START_ADDR + 1UL);
	uint32_t length_words = length_bytes / 4UL;

	return HAL_CRC_Calculate(&hcrc, (uint32_t *)APP_START_ADDR, length_words);
}

static uint32_t APP_ReadStoredCrc(void)
{
	return *(volatile uint32_t *)APP_CRC_ADDR;
}

bool APP_FlashCrcOk(void)
{
	uint32_t calc_crc   = APP_CalcCrc();
	uint32_t stored_crc = APP_ReadStoredCrc();
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
	...
	v4 = (float)SMon_P_NTC_PullUp_ResistorVale * ((float)Ain_DmaBuffer[4u] / ((float)SMon_P_ADC_MaxValue - (float)Ain_DmaBuffer[4u]));  // ntc
	...
	if(SMon_PeakCurrent < (v1 - SMon_P_NoLoad_ISense) * SMon_P_ConvFacISense)
	{
		SMon_PeakCurrent = (v1 - SMon_P_NoLoad_ISense) * SMon_P_ConvFacISense;
	}
	else
	{
		/* Do nothing. */
