...
DLLEXPORT s32 __stdcall retrieve_mp_abilities(const void* AObj, const TRegTSMasterFunction AReg) {
  #define TSMASTER_VERSION "2025.11.14.1719"
  ...
  if (!AReg(AObj, "on_custom_callback", "on_after_tsmaster_open", "void", reinterpret_cast<const void*>(&on_after_tsmaster_open), "demo user event handler for TSMaster open")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_before_tsmaster_close", "void", reinterpret_cast<const void*>(&on_before_tsmaster_close), "demo user event handler for TSMaster close")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_after_project_open", "void", reinterpret_cast<const void*>(&on_after_project_open), "demo user event handler for TSMaster project open")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_before_project_close", "void", reinterpret_cast<const void*>(&on_before_project_close), "demo user event handler for TSMaster project close")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_before_app_connect", "void", reinterpret_cast<const void*>(&on_before_app_connect), "demo user event handler for TSMaster application connect")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_after_app_connect", "void", reinterpret_cast<const void*>(&on_after_app_connect), "demo user event handler for TSMaster application connect")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_before_app_disconnect", "void", reinterpret_cast<const void*>(&on_before_app_disconnect), "demo user event handler for TSMaster application disconnect")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_after_app_disconnect", "void", reinterpret_cast<const void*>(&on_after_app_disconnect), "demo user event handler for TSMaster application disconnect")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_before_project_save", "void", reinterpret_cast<const void*>(&on_before_project_save), "demo user event handler for TSMaster project save")) return -1;
  if (!AReg(AObj, "on_custom_callback", "on_after_project_save", "void", reinterpret_cast<const void*>(&on_after_project_save), "demo user event handler for TSMaster project save")) return -1;
