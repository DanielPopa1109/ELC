...
extern void on_shortcut_QC_Test_Activate(const s32 AShortcut);
extern void on_shortcut_QC_Test_Deactivate(const s32 AShortcut);
DLLEXPORT s32 __stdcall retrieve_mp_abilities(const void* AObj, const TRegTSMasterFunction AReg) {
  ...
  if (!AReg(AObj, "on_shortcut_callback", "on_shortcut_QC_Test_Activate", "Ctrl+A", reinterpret_cast<const void*>(&on_shortcut_QC_Test_Activate), "")) return -1;
  if (!AReg(AObj, "on_shortcut_callback", "on_shortcut_QC_Test_Deactivate", "Ctrl+B", reinterpret_cast<const void*>(&on_shortcut_QC_Test_Deactivate), "")) return -1;
